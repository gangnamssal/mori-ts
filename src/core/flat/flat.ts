import { IterableRecurInfer } from './../../types';
import { isAsyncIterable, isIterable } from './../../utils';

function* syncFlat<A extends Iterable<unknown>>(iter: A): IterableIterator<IterableRecurInfer<A>> {
  for (const item of iter) {
    if (isIterable(item)) yield* syncFlat(item as A) as IterableIterator<IterableRecurInfer<A>>;
...
async function* asyncFlat<A extends AsyncIterable<unknown>>(
  ...
  for await (const item of iter) {
    if (isAsyncIterable(item)) yield* asyncFlat(item as A) as AsyncIterableIterator<IterableRecurInfer<A>>;
    else yield item as IterableRecurInfer<A>;
  }
}
...
function flat<A extends Iterable<unknown> | AsyncIterable<unknown>>(
  ...
  if (isIterable(iter)) return syncFlat(iter);
  if (isAsyncIterable(iter)) return asyncFlat(iter);
